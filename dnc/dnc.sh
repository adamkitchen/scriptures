#!/bin/sh
# dnc: Read the Word of God from your terminal
...
while [ $# -gt 0 ]; do
	...
	if [ "$1" = "--" ]; then
		...
		get_data dnc.tsv | awk -v cmd=list "$(get_data dnc.awk)"
		exit
	elif [ "$1" = "-W" ]; then
		export DNC_NOLINEWRAP=1
		shift
...
if [ $# -eq 0 ]; then
	...
	while true; do
		...
		get_data dnc.tsv | awk -v cmd=ref -v ref="$ref" "$(get_data dnc.awk)" | ${PAGER}
	done
	exit 0
fi

get_data dnc.tsv | awk -v cmd=ref -v ref="$*" "$(get_data dnc.awk)" | ${PAGER}
