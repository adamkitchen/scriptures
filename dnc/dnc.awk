BEGIN {
	...
	MAX_WIDTH = 80
	if (ENVIRON["DNC_MAX_WIDTH"] ~ /^[0-9]+$/) {
		if (int(ENVIRON["DNC_MAX_WIDTH"]) < MAX_WIDTH) {
			MAX_WIDTH = int(ENVIRON["DNC_MAX_WIDTH"])
		}
	}
...
function printverse(verse,    word_count, characters_printed) {
	if (ENVIRON["DNC_NOLINEWRAP"] != "" && ENVIRON["DNC_NOLINEWRAP"] != "0") {
		printf("%s\n", verse)
		return
	}
