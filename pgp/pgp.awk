BEGIN {
	...
	MAX_WIDTH = 80
	if (ENVIRON["PGP_MAX_WIDTH"] ~ /^[0-9]+$/) {
		if (int(ENVIRON["PGP_MAX_WIDTH"]) < MAX_WIDTH) {
			MAX_WIDTH = int(ENVIRON["PGP_MAX_WIDTH"])
		}
	}
...
function printverse(verse,    word_count, characters_printed) {
	if (ENVIRON["PGP_NOLINEWRAP"] != "" && ENVIRON["PGP_NOLINEWRAP"] != "0") {
		printf("%s\n", verse)
		return
	}
